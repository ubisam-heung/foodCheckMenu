...
# config_secret.py에서 민감 정보 import
from config_secret import MENU_PAGE_URL, IMG_DOMAIN_FILTER
...
def main():
    srcs = get_img_srcs(MENU_PAGE_URL)
    filtered = [src for src in srcs if IMG_DOMAIN_FILTER in src]
    img_url = None
    if len(filtered) >= 2:
        img_url = filtered[1]
    elif filtered:
